...
StellarObject::StellarObject(Vec3 center, Ogre::Vector3 rot, double colonization, Ogre::String name) {
	mName = name;
	_mesh = "sphere.mesh";
	_center = center;
	_rot = rot;
...
bool StellarObject::operator==(Ogre::String name) {
	return mName.compare(name) == 0;
}

Ogre::String& StellarObject::GetName(void) {
	return mName;
}